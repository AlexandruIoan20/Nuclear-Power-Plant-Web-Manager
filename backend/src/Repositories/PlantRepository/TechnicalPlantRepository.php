...
class TechnicalPlantRepository { 
    ...
    public function save(TechnicalPlantData $technicalPlantData): void { 
        try {
            LogService::instance()->debug("START");
            LogService::instance()->debug("TechnicalPlantData: " . print_r($technicalPlantData, true));
    
            $this->pdo->beginTransaction();
            LogService::instance()->debug("Transaction started");
            ...
            LogService::instance()->debug("INSERT technical_data params: " . print_r($insertParams, true));
    
            $statement->execute($insertParams);
            LogService::instance()->debug("technical_data inserted, rowCount: " . $statement->rowCount());
    
            $reactorConfigurations = $technicalPlantData->getReactorConfigurations(); 
            LogService::instance()->debug("reactorConfigurations count: " . count($reactorConfigurations));
            LogService::instance()->debug("reactorConfigurations raw: " . print_r($reactorConfigurations, true));
    
            $groupedConfigurations = [];
            foreach ($reactorConfigurations as $index => $config) {
                $key = $config->getType()->value . '_' . $config->getCooling()->value;
                LogService::instance()->debug("config[$index] key: $key | type: " . $config->getType()->value . " | cooling: " . $config->getCooling()->value);
                ...
                LogService::instance()->debug("config[$index] quantity for key '$key' now: " . $groupedConfigurations[$key]['quantity']);
            }
            LogService::instance()->debug("groupedConfigurations final: " . print_r($groupedConfigurations, true));
            ...
            foreach ($groupedConfigurations as $key => $group) { 
                ...
                LogService::instance()->debug("INSERT reactor_plant_data for key '$key': " . print_r($relationalParams, true));
    
                $relationalStatement->execute($relationalParams); 
    
                $rowCount = $relationalStatement->rowCount();
                LogService::instance()->debug("reactor_plant_data rowCount for key '$key': $rowCount");
    
                if ($rowCount === 0) {
                    LogService::instance()->debug("ERROR - reactor_schema not found for type: " . $group['type'] . " | cooling: " . $group['cooling']);
                    throw new Exception("Configurația reactorului (" . $group['type'] . " - " . $group['cooling'] . ") nu există în catalog.");
                }
    
                LogService::instance()->debug("reactor_plant_data inserted successfully for key '$key'");
            }
    
            $this->pdo->commit();
            LogService::instance()->debug("Transaction committed - DONE");
    
        } catch (Exception $e) { 
            $this->pdo->rollBack();
            LogService::instance()->debug("ROLLBACK triggered");
            LogService::instance()->debug("Eroare la salvare: " . $e->getMessage());
            LogService::instance()->debug("Stack trace: " . $e->getTraceAsString());
    ...
    public function update(TechnicalPlantData $technicalPlantData): void { 
        try {
            ...
            $this->pdo->rollBack();
            LogService::instance()->error("Eroare la actualizare: " . $e->getMessage());
